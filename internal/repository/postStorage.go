...
func (p *PostStorage) CreatePost(post models.Post) (int, error) {
	...
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("[PostStorage]:Error with CreatePost method in repository: %w", err)
	}

	_, err = query.Exec(post.Uuid, post.Title, post.Content, post.Author, post.CreatedAt, post.Categories)
	if err != nil {
		return http.StatusBadRequest, fmt.Errorf("[PostStorage]:Error with CreatePost method in repository: %w", err)
	}
...
func (p *PostStorage) GetPostIdWithUUID(uuid uuid.UUID) ([]int, error) {
...
func (p *PostStorage) GetUsersLikePosts(postIdArray []int) ([]models.Post, error) {
	result := []models.Post{}
